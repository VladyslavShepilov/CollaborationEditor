...
class Document:
    ...
    def delete(self, char: CharId) -> Char | None:
        if (delete_index := self.find_index(char)) is None:
            self.pending_delete.append(char)
            return None
    ...
    def _insert_after_parent(self, parent_index: int, new_char: Char) -> Char:
        ...
        self._char_index[new_char.char_id] = len(self.content) - 1
        return new_char
